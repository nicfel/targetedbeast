...
public class ScaleOperator2 extends TreeOperator {
	...
	public double proposal() {
    	...
    	for (int i = 0; i < tree.getNodeCount(); i++) {
			if (tree.getNode(i).isRoot())
				continue;			
			totalMutations += Math.min(10, rapidTreeLikelihoodInput.get().getEdgeMutations(i)) ;		
    	}
        double scaler = Randomizer.nextDouble() * totalMutations;
        int randomNode = -1;
        double currMuts = 0;
        for (int i = 0; i < tree.getNodeCount(); i++) {
        	currMuts +=  Math.min(10, rapidTreeLikelihoodInput.get().getEdgeMutations(i));
			if (currMuts > scaler) {
				randomNode = i;
				break;
			}
        }
        logHastingsRatio -= Math.log( Math.min(10, rapidTreeLikelihoodInput.get().getEdgeMutations(randomNode)) / totalMutations);
		...
		// calculate the distance between the consensus of i and the other nodes after removing i from the consensus
...
//		rapidTreeLikelihoodInput.get().prestore();
    	...
    	for (int k = 0; k < tree.getNodeCount(); k++) {
			if (tree.getNode(k).isRoot())
				continue;			
			totalMutations +=  Math.min(10, rapidTreeLikelihoodInput.get().getEdgeMutations(k));	
    	}
...
//    	System.out.println(logHastingsRatio);
    	// check if there are any negative branch lengths    	
		for (int k = 0; k < tree.getNodeCount(); k++) {
			if (tree.getNode(k).getLength() < 0) {
				System.exit(0);
				return Double.NEGATIVE_INFINITY;
			}
		}
		
		if (rapidTreeLikelihoodInput.get().getLogP() >-12000) {
			System.out.println(rapidTreeLikelihoodInput.get().getLogP());
			System.exit(0);
//			return Double.NEGATIVE_INFINITY;
		}
	...
	public void replace(final Node node, final Node child, final Node replacement) {
		...
		node.makeDirty(2 - node.isDirty());
		replacement.makeDirty(2 - replacement.isDirty());

		if (replacement.isDirty() == 3) {
			System.out.println("dirty");
		} else if (node.isDirty() == 3) {
			System.out.println("dirty");
		}
