...
@Description("Moves the height of an internal node along the branch. " +
        ...
        "slide down into.")
public class SubtreeSlide5 extends TreeOperator {
    ...
    public final Input<Integer> numberOfAttemptsInput = new Input<>("attempts", "number of attempts to make before giving up", 50);

    public Input<RealParameter> clockRateInput = new Input<>("clockRate", "the rate to scale the branch lengths with. If not specified, the branch lengths are not scaled.");
    ...
    public void initAndValidate() {
        ...
        numberOfAttempts = numberOfAttemptsInput.get();
    }
    ...
    public double proposal() {
        ...
        double logq = 0.0;
        double mean = getDelta();
        
        for (int a=0; a<numberOfAttempts; a++) {
	        // choose a random double between 0 and tree length
	        double scaler = Randomizer.nextDouble() * treeLength;
 
	        // get the first node for which total length is greater than scaler
	        double totalLength = 0;
	        int nodeNr = -1;
	        
	    	for (int i = 0; i < tree.getNodeCount(); i++) {
				totalLength += tree.getNode(i).getLength();
				if (totalLength > scaler) {
					nodeNr = i;
					break;
				}
	    	}
	    	
	        Node node = tree.getNode(nodeNr).getParent();
	        
			if (node.isRoot()) {
				return Double.NEGATIVE_INFINITY;
			}

        	logq += Math.log(tree.getNode(nodeNr).getLength()/treeLength);    
        	
            logq += doStep(tree, mean, node);  
            
            
            
            // calculate tree length TODO can be done more efficiently
            double newLength = 0.0;
        	for (int i = 0; i < tree.getNodeCount(); i++) {
    			if (tree.getNode(i).isRoot())
    				continue;
    			
    			newLength += tree.getNode(i).getLength();
        	}

            logq -= Math.log(tree.getNode(nodeNr).getLength()/(newLength));
    ...
    private double doStep(Tree tree, double mean, Node i) {
    	double logq = 0.0;
...
//        }
//        
//        // pick a node based on the number of descendents
//        double height = Randomizer.nextDouble() * totDescendents;
//        
//        int descCount = 0;
//        for (int j = 0; j < nodeCount; j++) {
//			descCount += descendents[j];
//			if (descCount > height) {
//				i = tree.getNode(j);
//				break;
//			}
//        }
        ...
        final double delta = getDelta() + mean;
...
//                p.getParent().makeDirty(3-p.getParent().isDirty()); // JH
        ...
        // 4 if we are sliding the subtree down.
            ...
            if (CiP.getHeight() > newHeight) {
                ...
                if( markClades ) {
                    ...
                    Node n = p;
                    while( n != CiP ) {
                        n.makeDirty(Tree.IS_FILTHY - n.isDirty()); // JH
                        n = n.getParent();
                    }
                }
