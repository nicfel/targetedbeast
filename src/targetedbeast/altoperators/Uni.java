...
@Description("Moves the height of an internal node along the branch. " +
        ...
        "slide down into.")
public class Uni extends TreeOperator {

    final public Input<Double> sizeInput = new Input<>("size", "size of the slide, default 1.0", 0.001);
    ...
    public double proposal() {
        ...
        logq += addToHeight(tree.getRoot(), mean);
    ...
    private double addToHeight(Node n, double mean) {
		if (n.isLeaf()) {
		...
		n.setHeight(n.getHeight() + mean);
		for (Node child : n.getChildren()) {
			addToHeight(child, mean);
		}
		return 0;
	}
    ...
    private double doStep(Tree tree, double mean, Node p) {
    	double logq = 0.0;
