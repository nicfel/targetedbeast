...
public class RapidFilteredAlignment extends RapidAlignment {
    ...
    final public Input<RapidAlignment> alignmentInput = new Input<>("data", "alignment to be filtered", Validate.REQUIRED);
    ...
    public void initAndValidate() {
        parseFilterSpec();
        calcFilter();
        RapidAlignment data = alignmentInput.get();
    ...
    protected void calcPatterns() {
        ...
        int[][] data = new int[nrOfSites+stateCounts.get(0)][nrOfTaxa];
        ...
        for (int i = 0; i < nrOfTaxa; i++) {
			...
			for (int j = 0; j < stateCounts.get(i); j++) {
				data[j + nrOfSites][i] = j;
			}
        }
        ...
        // remove -1 from the first stateCounts
