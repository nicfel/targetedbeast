...
//		coExistingNodes.remove(coExistingNodes.indexOf(CiP.getNr()));
		...
		// reverse target term recomputed post-move below (on the post-move tree without i)	      
        ...
        // on the post-move tree WITHOUT i (mirror of the forward selection, evaluated in state B).
        {
            ...
            List<Node> revCo = getCoExistingLineages(i, tree);
            List<Integer> revAnc = new ArrayList<>();
            for (Node a = i.getParent(); a != null; a = a.getParent())
                if (revCo.contains(a)) revAnc.add(a.getNr());
            edgeWeights.prestore();
            edgeWeights.updateByOperatorWithoutNode(i.getNr(), revAnc);
            revCo.remove(newP);
            revCo.remove(i);
            int cipIdx = revCo.indexOf(CiP);
            int sibIdx = revCo.indexOf(newSib);
            if (cipIdx < 0 || sibIdx < 0) { edgeWeights.reset(); return Double.NEGATIVE_INFINITY; }
            double[] revDist = edgeWeights.getTargetWeights(i.getNr(), revCo);
            double cipD = Math.pow(revDist[cipIdx], 2);
            revDist[sibIdx] = 0;
            double revTotal = 0;
            for (int k = 0; k < revCo.size(); k++) { revDist[k] = Math.pow(revDist[k], 2); revTotal += revDist[k]; }
            edgeWeights.reset();
            logHastingsRatio += Math.log(cipD / revTotal);
        }
...
//		coExistingNodes.remove(coExistingNodes.indexOf(CiP.getNr()));
		...
		logHastingsRatio -= Math.log(distance[nodeNr] / totalDistance);
		// reverse target term recomputed post-move below (post-move tree without i)
		...
		// on the post-move tree WITHOUT i (mirror of the forward selection, evaluated in state B).
		{
			Node newP = i.getParent();
			Node newSib = getOtherChild(newP, i);
			double revMinHeight = i.getHeight();
			List<Integer> revCo = new ArrayList<>();
			for (int k = 0; k < tree.getNodeCount(); k++) {
				if (tree.getNode(k).isRoot()) continue;
				if (tree.getNode(k).getParent().getHeight() > revMinHeight) revCo.add(k);
			}
			List<Integer> revAnc = new ArrayList<>();
			for (Node a = i.getParent(); a != null; a = a.getParent())
				if (revCo.contains(a.getNr())) revAnc.add(a.getNr());
			edgeWeights.prestore();
			edgeWeights.updateByOperatorWithoutNode(i.getNr(), revAnc);
			revCo.remove(Integer.valueOf(newP.getNr()));
			revCo.remove(Integer.valueOf(i.getNr()));
			int cipIdx = revCo.indexOf(CiP.getNr());
			int sibIdx = revCo.indexOf(newSib.getNr());
			if (cipIdx < 0 || sibIdx < 0) { edgeWeights.reset(); return Double.NEGATIVE_INFINITY; }
			double[] revDist = edgeWeights.getTargetWeightsInteger(i.getNr(), revCo);
			double cipD = Math.pow(revDist[cipIdx], 2);
			revDist[sibIdx] = 0;
			double revTotal = 0;
			for (int k = 0; k < revCo.size(); k++) { revDist[k] = Math.pow(revDist[k], 2); revTotal += revDist[k]; }
