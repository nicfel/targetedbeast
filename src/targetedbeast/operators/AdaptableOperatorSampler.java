...
public class AdaptableOperatorSampler extends Operator {
    ...
    final public Input<Integer> maxMovesInput = new Input<>("maxMoves", "number of operator calls until the learning process begins (default: 1000)", Integer.MAX_VALUE);
...
//	String treeStrBefore;
	@Override
	public double proposal() {
		if (count>maxMovesInput.get())
...
//		treeStrBefore = rapidTreeLikelihoodInput.get().getTree();
		...
		if (learningHasBegun) {
			...
			try {
				...
				if (this.numParams > 0) {
					...
					squaredDiffs = this.computeSS(this.stateBefore, stateAfter, this.treesBefore, this.trees);
					...
					if (squaredDiffs[0]>50) {
//						System.out.println(lastOperator + " " +(stateAfter[0][0]-stateBefore[0][0]) + " " + this.getID());
//						System.out.println(lastOperator + " " +this.lastOperator);
//						System.out.println(treeStrBefore +";");
