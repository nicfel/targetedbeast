...
public class NodeRandomizer22 extends TreeOperator {
    ...
    final public Input<Boolean> optimiseInput = new Input<>("optimise",
			"flag to indicate that the scale factor is automatically changed in order to achieve a good acceptance rate (default false)",
			false);
	...
	public double proposal() {
		...
		for (int i = 0; i < nrOfAttempts; i++) {
			logHastingsRatio += singleProposal();
...
//		}
    ...
    public void optimize(final double logAlpha) {
        if (optimiseInput.get()) {
        	...
        	percentage = Math.min(Math.max(f, lower), 1.0);
        }
    }
