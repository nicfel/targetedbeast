...
render_matrix <- function(csv, png, mdout = NULL) {
  ...
  png(png, width = wi, height = hi, units = "in", res = 150, type = "cairo", bg = "white")
...
render_distributions <- function(logdir, outdir) {
  ...
  for (st in STATS) {
    ...
    png(file.path(outdir, paste0("rateprior_", gsub("\\.", "_", st), ".png")),
        width = 15, height = 2.4 * nr + 0.6, units = "in", res = 120, type = "cairo", bg = "white")
...
render_balance_scatter <- function(csv, png) {
  ...
  d <- fread(csv); d[, compared := as.logical(compared)]
  tests <- unique(d$test)
  n <- length(tests); nc <- 5; nr <- ceiling(n / nc)
  png(png, width = 15, height = 2.9 * nr + 0.4, units = "in", res = 120, type = "cairo", bg = "white")
  ...
  for (t in tests) {
    ...
    points(fin$forward, fin$backward, col = "steelblue", pch = 19, cex = 0.7)      # hit + balanced
    bad <- fin[abs(forward - backward) > tol]                                      # off-diagonal beyond 3σ
    if (nrow(bad)) points(bad$forward, bad$backward, col = "firebrick", pch = 19, cex = 1.0)
